...
// ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═╝╚══════╝    ╚═════╝  ╚═════╝    ╚═╝   
//
// KARAOKE BOT ULTIMATE EDITION v3.4 (JSONBin + Pagination + Skip Uploaded)
// - ระบบ Lock/Unlock ด้วย Key (Admin เท่านั้น) — จำสถานะได้
// - ระบบ Fallback 3 ชั้นสำหรับดาวน์โหลด
// - ระบบเลือกอัปโหลดเพลงขึ้น Roblox (ข้ามเพลงที่เคยอัปแล้ว)
...
client.once('ready', async () => {
    ...
    const commands = [
        ...
        new SlashCommandBuilder().setName('อัปโหลดทั้งหมด').setDescription('อัปโหลดเพลงที่ยังไม่เคยอัปโหลดขึ้น Roblox').setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
...
client.on('interactionCreate', async interaction => {
    ...
    // /อัปโหลด (แก้ใหม่: แสดงเฉพาะที่ยังไม่อัปโหลด)
    if (commandName === 'อัปโหลด') {
        const allSongs = Object.values(songs);
        const pending = allSongs.filter(s => !s.robloxAssetId);
        const alreadyUploaded = allSongs.length - pending.length;
        
        if (pending.length === 0) {
            return interaction.reply({ 
                embeds: [replyEmbed
                    .setTitle('✅ ทุกเพลงอัปโหลดแล้ว!')
                    .setDescription(`ในคลังมี **${allSongs.length}** เพลง\nอัปโหลดแล้วทั้งหมด **${alreadyUploaded}** เพลง`)
                    .setColor(0x57F287)] 
            });
        }
        ...
        await interaction.reply({
            embeds: [new EmbedBuilder().setTitle('📤 เลือกเพลงเพื่ออัปโหลด')
                .setDescription(
                    `📦 เพลงในคลัง: **${allSongs.length}** เพลง\n` +
                    `🟢 อัปโหลดแล้ว (ไม่แสดง): **${alreadyUploaded}** เพลง\n` +
                    `🔴 ยังไม่อัปโหลด: **${pending.length}** เพลง\n\n` +
    ...
    // /อัปโหลดทั้งหมด (แก้ใหม่: ข้ามเพลงที่เคยอัปโหลดแล้ว)
    if (commandName === 'อัปโหลดทั้งหมด') {
        ...
        await interaction.editReply({
            embeds: [new EmbedBuilder()
                ...
                .setDescription(
                    `**จะอัปโหลดเฉพาะเพลงที่ยังไม่เคยอัปโหลด**\n\n` +
                    `📦 เพลงในคลังทั้งหมด: **${allSongs.length}** เพลง\n` +
                    `🟢 อัปโหลดแล้ว (จะข้าม): **${alreadyUploaded}** เพลง\n` +
                    `🔴 ยังไม่อัปโหลด (จะอัปโหลด): **${pending.length}** เพลง\n\n` +
...
async function handleComponents(interaction) {
    ...
    if (interaction.customId === 'select_upload') {
        ...
        for (let i = 0; i < selectedIds.length; i++) {
            ...
            // ✅ ถ้าเพลงนี้เคยอัปโหลดแล้ว → ข้าม
            if (song.robloxAssetId) {
                results.skipped++;
                continue;
            }
        ...
        await interaction.editReply({
            embeds: [new EmbedBuilder().setTitle('✅ อัปโหลดเสร็จสิ้น')
                .addFields(
                    ...
                    { name: '⏭️ ข้าม', value: `${results.skipped}`, inline: true }
                )
                .setColor(0x57F287)]
        });
    }
    ...
    if (interaction.customId === 'confirm_upload_all') {
        ...
        for (let i = 0; i < pending.length; i++) {
            ...
            await interaction.editReply({
                embeds: [new EmbedBuilder()
                    .setTitle(`📤 กำลังอัปโหลด ${i+1}/${pending.length}`)
                    .setDescription(
                        `🎵 **${song.title}**\n🎤 ${song.artist}\n\n` +
                        `${progressBar(i+1, pending.length)}\n\n` +
                        `📊 สำเร็จ: ${results.success} | ล้มเหลว: ${results.failed}`
                    )
                    .setColor(0x3498db).setThumbnail(song.thumbnail)],
                components: []
            });
        ...
        await interaction.editReply({
            embeds: [new EmbedBuilder()
                ...
                .setDescription(
                    `📦 เพลงในคลัง: **${allSongs.length}** เพลง\n` +
                    `⏭️ ข้าม (เคยอัปโหลดแล้ว): **${alreadyUploaded}** เพลง\n` +
                    `📤 พยายามอัปโหลด: **${pending.length}** เพลง`
                )
                .addFields(
                    { name: '🟢 สำเร็จ', value: `${results.success}`, inline: true },
                    { name: '🔴 ล้มเหลว', value: `${results.failed}`, inline: true }

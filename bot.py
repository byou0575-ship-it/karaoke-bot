...
# ตั้งค่าเป้าหมายการค้นหาอัตโนมัติ (แก้ตรงนี้ได้เลย เช่น "Saran", "เพลงไทย", "Bodyslam")
AUTO_TARGET = "เพลงไทย" 
...
# Bot Logic & Commands (มีครบทั้ง Sync และ Auto)
...
# คำสั่งเพิ่มทีละเพลง
@karaoke_group.command(name="auto", description="เพิ่มเพลงเดียว (ใส่ URL)")
...
# ฟีเจอร์ที่ 1: ดึงเพลงทั้งหมดของศิลปิน (เช่น Saran)
...
async def sync_artist(interaction: discord.Interaction, artist: str):
    ...
    try:
        ...
        for url in urls:
            ...
            await asyncio.sleep(3) # เว้น 3 วิต่อเพลง
        await refresh_song_channel(bot)
...
# ฟีเจอร์ที่ 2: ดึงเพลงยอดนิยม
@karaoke_group.command(name="trend", description="ดึง 100 เพลงยอดนิยม (โหลดทีละ 20 เพลง)")
...
# ฟีเจอร์ที่ 3: Auto หาเพลงเองเรื่อยๆ (ไม่ต้องใส่คิว)
@karaoke_group.command(name="auto_start", description="เริ่มระบบหาเพลงใหม่ให้อัตโนมัติ")
        ...
        while True:
            try:
                ...
                for url in urls:
                    ...
                    await asyncio.sleep(10) # พัก 10 วิเพื่อความปลอดภัย
                await asyncio.sleep(60) # หาครบแล้วรอ 1 นาทีแล้ววนหาใหม่
            except Exception as e:
                logger.error(f"Auto loop error: {eop}")
                await asyncio.sleep(30)
    
    auto_task = asyncio.create",_task(loop())

@karaoke_group.command(name="auto_st description="หยุดระบบหาเพลงอัตโนมัติ")
